...
function update() {
  if (
    game.resources.availableHousing != 0 ||
    game.work.farmers + game.work.lumberjack != 0
  ) {
  ...
  if (previousValues.foodAmount != game.resources.food) {
    ...
    perSecound =
      game.work.farmers - game.work.lumberjacks * 3 - game.resources.unassigned;
    document.getElementById("foodAmount").innerText =
      "Food: " + game.resources.food + ` (${perSecound}/s)`;
    check();
  }
  if (previousValues.woodAmount != game.resources.wood) {
    ...
    document.getElementById("woodAmount").innerText =
      "Wood: " + game.resources.wood + ` (${perSecound}/s)`;
    check();
  }
  if (previousValues.housingAmount != game.resources.availableHousing) {
    ...
    document.getElementById("housingAmount").innerText =
      "Available Homes: " + game.resources.availableHousing;
    check();
  }
  if (previousValues.unassigned != game.resources.unassigned) {
    previousValues.unassigned = game.resources.unassigned;
    document.getElementById("unassigned").innerText =
      "Unassigned: " + game.resources.unassigned;
    check();
  }
  if (previousValues.farmers != game.work.farmers) {
    previousValues.farmers = game.work.farmers;
    document.getElementById("farmers").innerText =
      "Farmers: " + game.work.farmers;
    check();
  }
  if (previousValues.lumberjacks != game.work.lumberjacks) {
    previousValues.lumberjacks = game.work.lumberjacks;
    document.getElementById("lumberjacks").innerText =
      "Lumberjacks: " + game.work.lumberjacks;
    check();
  }
}
...
function check() {
  ...
  for (var loop = 0; loop < game.createable.length; loop++) {
    ...
    if (
      game.resources.food >= game.createable[loop].foodCost &&
      game.resources.wood >= game.createable[loop].woodCost
    ) {
      createcreateable(
        game.createable[loop].amount,
        game.createable[loop].name,
        loop,
        game.createable[loop].woodCost
      );
    }
  }
}
...
function create(type) {
  ...
  for (var loop = 0; loop < game.createable.length; loop++) {
    ...
    houses +=
      game.createable[loop].amount * game.createable[loop].housingBenefit;
  }
  game.resources.availableHousing =
    houses -
    (game.resources.unassigned + game.work.farmers + game.work.lumberjacks); // Calculates the avalable houses by removing the the game.workers from total housing
  update();
  check();
}
...
function createcreateable(amount, name, id, price) {
  ...
  document.getElementById(
    "expandable"
  ).innerHTML += `<button onclick=create(${id})>Buy ${name} (${price} wood)</button><br>`;
